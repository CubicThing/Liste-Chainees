...
int insListeVide (Liste * liste, int *donnee, int *donnee2){

  Element *nouveau_element;

  if ((nouveau_element = (Element *) malloc (sizeof (Element))) == NULL)
    return -1;
  if ((nouveau_element->p.x = (char *) malloc (50 * sizeof (char)))
      == NULL)
    return -1;

   nouveau_element->p.x = donnee;
   nouveau_element->p.y = donnee2;
...
int insDebutListe (Liste * liste, int *donnee1, int *donnee2){
 ...
 Element *nouveau_element;
  ...
  strcpy (nouveau_element->p.x, donnee1);
  strcpy (nouveau_element->p.y, donnee2);
...
int insFinListe (Liste * liste, Element * courant, char *donnee){
  ...
  strcpy (nouveau_element->p.x, donnee);
...
int suppDebut (Liste * liste){
  ...
  if (liste->taille == 1)
    liste->fin = NULL;
  free (supp_element->p.x);
  free (supp_element->p.y);
  free (supp_element);
  liste->taille--;
  return 0;
}
...
void affiche (Liste * liste){
  ...
  courant = liste->debut;
  while (courant != NULL){
      printf ("%p - %d,%d\n", courant, courant->p.x, courant->p.x);
      courant = courant->suivant;
  }
}
